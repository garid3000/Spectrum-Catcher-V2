...
def init():
    ...
    currentExpo = 'Expo: {:4d}'.format(v4l2.possible_expos[v4l2.get_expo_i()])

    bm.stdscr.addstr(1, 9, "Setup - Exposure Control")
    #bm.addButton(1, 9, 3, 14, "Setting Expo", tag="mb1", buttonId="mb1_1", show=True)
    bm.addButton(4, 9, 3, 10, "+ expo",       tag="mb1", buttonId="mb1_2", show=True)
    bm.addButton(4, 19,3, 10, "- expo",       tag="mb1", buttonId="mb1_3", show=True)
...
def callback_expo_up():
    v4l2.set_expo_i(v4l2.expo_i + 1)
    i= v4l2.get_expo_i()
    bm.setLabel('mb1_4', 'Expo: {:4d}'.format(v4l2.possible_expos[i]))
...
def main(stdscr):
    ...
    #stdscr.box()
    rectangle(stdscr, 1, 3, 14, 35)

    #time.sleep(1)
    init()
    stdscr.refresh()
    #stdscr.addstr(1,1, "asdfadsfasdfa")
    bm.refresh(1,1)
    while 1:
        y,x = getTouch()
        bm.refresh(y, x)
